...
if rg -n "src=\"/assets|href=\"/assets|href=\"/vite.svg\"" frontend/dist/index.html >/dev/null; then
...
if [[ ! -x "backend/bin/signalhub-backend" ]]; then
...
if ! ls backend/whisper_cache/whisper/*.pt >/dev/null 2>&1; then
...
if ! rg -n "frontend_dist" desktop/electron-builder.yml >/dev/null; then
  fail "electron-builder.yml missing frontend_dist in extraResources"
fi
if ! rg -n "whisper_cache" desktop/electron-builder.yml >/dev/null; then
